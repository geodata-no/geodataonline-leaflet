...
L.AgsDynamicLayer = L.Class.extend({
    ...
    _getImageUrl: function () {
        ...
        //JJ
        var sr = "3857";
        if (this._map.options.crs && this._map.options.crs.code) {
            sr = this._map.options.crs.code.split(":")[1];
        }
        ...
        var ne = this._map.options.crs.project(bnds._northEast),
            sw = this._map.options.crs.project(bnds._southWest);
        var bbox = 'bbox=' + [sw.x, sw.y, ne.x, ne.y].join(',') + '&bboxsr=' + sr + '&imageSR=' + sr;
    ...
    _updateLayer: function () {
        if (!this._image.updating) {
            ...
            var topLeft = this._map.latLngToLayerPoint(this._map.getBounds().getNorthWest()),
                ...
                size = bottomRight.subtract(topLeft);
            */
            var topLeft = this._map.containerPointToLayerPoint(L.point(0, 0)),
